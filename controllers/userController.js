...
userController.verifyUser = async (req, res, next) => {
  ...
  User.findOne({ username: username }, (err, user) => {
    ...
    // if user found, check passed in password aganist hashed password
    bcrypt.compare(password, user.password, (err, result) => {
      // if err with bcrpyt.compare
      if (err) {
        return next(createErr({err: err, method: 'verifyUser', message: err.message || 'Error occurred in verifyUser: Couldn\'t verify user.'}));
      }
      // if passed password and hashed password do not match 
      if (!result) {
        return res.status(404).json({error: `Invalid credentials`});
      }
      // if passed password and hashed password do match 
      return next();
    });
  });
}
